...
public:
    ...
    void animate(int limitXL, int limitXR, int limitYU,
                 int limitYD, SDL_Rect cameraRect,
                 SoundManager &soundManager);
    Unit* selectUnit(int dx1, int dx2, int dy1,
                     int dy2, bool &found, int id_client);
    Unit* selectEnemy(int dx1, int dx2, int dy1,
                      int dy2,Action &action, int id_client);
    ...
    std::vector<Unit*> selectUnits(int rangeX1, int rangeX2,
                                   int rangeY1, int rangeY2,
                                   bool &found, int id_client);
    Unit* getFortPlayer(int idClient);

private:
    void createUnit(int cod_unit,int unit_owner,
                    int posX, int posY, Factory_Units &factory,
                    FlagsUnitType blue, FlagsUnitType green,
                    FlagsUnitType red, FlagsUnitType yellow);
